...
const nav: navProps[] = [
  {
    ...
    icon: ({ className }: IconProps) => (
      <CalendarDays
        className={cn(className, 'stroke-white size-7')}
        fill="#f382ec"
      />
    ),
    iconFilled: ({ className }: IconProps) => (
      <CalendarDays
        className={cn(className, 'stroke-primary size-7')}
        fill="white"
      />
    )
  },
  ...
  */
  ...
  }, */
  {
    ...
    main: false,
    icon: ({ className }: IconProps) => (
      <User2 className={cn(className, 'stroke-white size-7')} fill="#f382ec" />
    ),
    iconFilled: ({ className }: IconProps) => (
      <User2 className={cn(className, 'stroke-primary size-7')} fill="white" />
    )
  }
]
  ...
  return (
    <>
      ...
      <div className="fixed bottom-0 left-0 w-full bg-white grid grid-cols-2 p-5 gap-x-10">
        {nav.map((data, index) => (
          <BottomNavLink {...data} key={index} iconFilled={data.iconFilled} />
        ))}
        <ElevatedButton />
      </div>
    </>
    ...
    <>
      ...
      >
        {active ? (
          ...
          <IconFilled className="size-6" />
        )}
        <span className={cn('text-sm', active && 'font-bold')}>{text}</span>
      </Link>
    </>
  )
}
