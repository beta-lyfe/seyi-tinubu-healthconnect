...
const LoginPageContext = createContext<{
  isPatient: boolean
  setPatient: Function
}>({} as { isPatient: boolean; setPatient: Function })
...
function Header() {
  return (
    <div className="flex flex-col gap-3 justify-center">
      <Typography.Title className="text-primary text-[30px] text-center">
        Login
      </Typography.Title>
      <Typography.Info className="text-black">
...
function Choose() {
  ...
  return (
    <div className="flex w-[100%] flex-row rounded-2xl border-primary border-2 cursor-pointer">
      <div
        className={'flex flex-1 py-2  rounded-lg items-center justify-center rounded-r-none text-center '.concat(
          context?.isPatient ? 'bg-primary' : 'bg-transparent'
        )}
        onClick={() => context?.setPatient(true)}
      >
        <Typography.Info
          className={context?.isPatient ? 'text-white' : 'text-primary'}
        >
          Patient
        </Typography.Info>
      </div>
      <div
        className={'flex flex-1 py-2 rounded-lg justify-center items-center rounded-l-none '.concat(
          !context?.isPatient ? 'bg-primary' : 'bg-transparent'
        )}
...
const Form = () => {
  ...
  return (
    <form
      ...
      onSubmit={() =>
        navigate.navigate({ to: isPatient ? '/dashboard' : '/doctor' })
      }
    >
  ...
  return (
    <div className="flex flex-col">
      <Typography.Info className="text-black opacity-40 text-sm">
        {!context?.isPatient
          ? `Yet to apply as a Listed Medical Doctor`
