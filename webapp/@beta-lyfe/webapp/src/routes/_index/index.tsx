...
function Hero(){
    ...
    <section className='lg:px-[70px] md:px[50px] md:py-[20px] py-[4px] bg-primary'>
        ...
        <Typography.Heading
        className='text-white text-2xl md:text-3xl text-left leading-normal'
        >
        Welcome to BetaLyfe <br />Your Health, Your Life, Your Way
        </Typography.Heading>
         <Typography.Info
        className='text-white text-md text-left leading-normal'
        >Transforming Healthcare with Cutting-Edge Telemedicine , 
        Say goodbye to the hassle of traditional doctor's visits. With BetaLyfe, you can schedule appointments, consult with healthcare providers,
         and access your medical records—all from your smartphone, tablet, or computer
        </Typography.Info>
        ...
        <img src='images/doctors/maria-watts.png'/>     
        </div>
    ...
    return(
        <nav className="md:hidden">
          <div className="flex justify-between items-center">
            ...
            <div className={`fixed top-0 left-0 w-full h-full bg-white transition-transform transform ${isOpen ? 'translate-x-0' : '-translate-x-full'} md:relative md:translate-x-0 md:flex items-center`}>
          <div className="flex p-8 flex-col md:flex-row w-full md:w-auto md:items-center md:ml-4 space-y-4 md:space-y-0 md:space-x-4 md:p-0">
            ...
            {links.map((item,id)=>
              <a href="#" key={id} className="block text-primary py-2 hover:text-gray-300">
              {item}
            </a>
            )}
          </div>
...
function WhatWeDo(){
    const aboutList=[{
        image:"images/icons8-appointments-64.png",
    ...
    {
        image:"images/icons8-examination-50.png",
        heading:"Expert Care",
    ...
    {
        image:"images/icons8-medical-profile-50.png",
    ...
    return(
        <section className='p-8 lg:p-[70px] md:px[50px]'>
            ...
            <p className='lg:w-[600px] text-left text-lg py-6'>Easy accessiblity to healthcare through information Technology
            From primary care and mental health services to chronic disease management and 
                prescription refills, BetaLyfe offers a wide range of healthcare services designed 
                to meet all your needs.</p>
            <div className='py-8'>
                <div className='grid md:grid-cols-3 gap-10'>
                    {aboutList.map((item,id)=>(
                        <AboutCards src={item.image} heading={item.heading} body={item.body}/>
                    ))}
                </div>
            </div>
            <div className='flex items-center justify-center pt-8'>
            <div className='grid gap-20 grid-flow-row md:grid-cols-2'>
                {statements.map((item,id)=><Statement heading={item.heading} body={item.body} key={id}/>)}
            </div>
            </div>
        </section>
    )
...
function Nav(){
    ...
    return(
        <nav className='lg:px-8 md:px-2 md:flex-[0.7] md:flex md:items-center justify-between gap-6 hidden'>
            ...
            <Link to='/login'>
            <Button className='bg-white text-primary hover:text-primary hover:bg-white'>Login</Button></Link>
        </nav>
    )
}

function AboutCards({src,heading,body}:{
    src:string,heading:string,body:string
}){
    return(
        <div className='p-6 shadow-lg rounded-xl'>
            <img src={src} className='w-[35px] '/>
            <h2 className='text-md font-semibold pt-3 pb-1 border-b-2 border-primary
            w-max
            '>{heading}</h2>
...
function MeetOurDoctors(){
    return(
        <section className='p-8 lg:px-[70px] md:px[50px] flex flex-col items-center gap-6'>
            <h2 className='text-3xl text-center md:text-left'>Meet Our Expert Doctors</h2>
            <p className='text-center'>Connect with a specialist for a 2nd Opinion . Access health and wlness Services</p>
            <div className='grid lg:grid-cols-3 md:grid-cols-2 gap-10 md:gap-20'>
                {doctors.map((item,id)=><DoctorCard src={item.image}
                key={id}
                name={`Dr. ${item.firstName} 
                ${item.lastName}`}
...
function DoctorCard({src,name,speciality}:{src:string,name:string,speciality:string}){
    return(
...
function Statement({heading,body}:{heading:string,body:string}){
    return(
        <div className='p-8 rounded-full border-primary border-2 text-center w-[350px]
...
function PartnerWithUs(){
    return(
        <section className='p-8 md:p-[70px] flex flex-col gap-4' >
            ...
            <p>One Score a a time. Betalyfe is making healthcare seamless for Nigerians living on Medication</p>
            <ul>
                <li>- 14- labs</li>
                <li>- 50+ world className doctors</li>
                <li>- 100- pharmacies</li>
            </ul>
            <p>By Q4 of 2024</p>
            <div className='text-center'>
                <Button className='rounded-r-full px-6 font-semibold'>Join<ArrowRight /></Button>
            </div>
        </section>
    )
}
        ...
        <div className="container mx-auto px-5 md:px-0">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                <div>
                    ...
                    <p>Stay updated with our latest news and offerings. Subscribe to our newsletter.</p>
                    <form className="mt-4">
                        <div className="flex items-center">
                            <input type="email" placeholder="Your email" className="w-full p-2
                            border-white border-2
                            rounded-l-lg border-none focus:ring-2 focus:ring-indigo-500" />
                            <button type="submit" className="bg-primary border-white border-[1px] p-2 rounded-r-lg transition-colors">Subscribe</button>
                        </div>
                    </form>
                </div>
                <div>
                    ...
                    <ul className='grid grid-cols-3 md:grid-cols-1'>
                        {
                            links.map((item,id)=> <li key={id} 
                            className="mb-2"><a href="#" className="hover:underline">{item}</a></li>)
                        }
                    </ul>
                </div>
                <div>
                    ...
                    <ul className="flex space-x-4">
                        <li>
                            <a href="https://twitter.com" className="hover:underline">
                                <svg className="w-6 h-6 fill-current text-blue-400" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                        ...
                        <li>
                            <a href="https://facebook.com" className="hover:underline">
                                <svg className="w-6 h-6 fill-current text-blue-600" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
            ...
            <div className="text-center mt-10">
                <p className="text-gray-500">&copy; 2024 Betalyfe. All rights reserved.</p>
            </div>
        </div>
    </footer>
    )
