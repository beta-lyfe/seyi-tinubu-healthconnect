...
const scheduleData=[
  ...
  },
  {
      ...
      "Description": "Engage in light exercises as prescribed by the physiotherapist",
      "Time": "5:00pm"
  },
  {
...
function DateHeader(){
  ...
  return(
    <header className="py-4">
      ...
      <Datepicker className="w-max md:w-max border-none outline-none" theme={DateTheme} weekStart={0}
      ...
      <Typography.Info className="font-bold text-3xl text-black pt-1">Today</Typography.Info>
    </header>
  )
}
...
function Calendar(){
  ...
  const DayListItem=(props:{day:string,dayno:number})=>{
    ...
    return (
  <div className="flex flex-col item-center"  onMouseOver={()=>setHighlighted(true)}
  onMouseLeave={()=>setHighlighted(false)}
  onClick={(e)=>e.preventDefault()}
  >
  <p className={cn("text-center opacity-75",highlighted && "text-primary")}>{props.day}</p>
  <div className="flex flex-col justify-center items-center">
    <p className={cn("text-center font-bold",highlighted && "text-primary")}>{props.dayno}</p>
  <div className={cn("w-1 h-1 rounded-full",highlighted && "bg-primary")}></div>
  </div>
  </div>)}
    ...
    <div className="py-4">
      {
        ...
        <ScheduleItem highlighted={highlighted==index} 
        {...item}
        index={index} key={index} setHighlighted={setHighlighted}/>
        )
      }
    </div>
  )
...
function ScheduleItem({highlighted,setHighlighted,index,Title:title,Time:time,Description:desc}:
  {highlighted:boolean,setHighlighted:Function,index:number,Title:string,Description:string,Time:string}){
  return(
    <div className="h-fit grid grid-cols-10 gap-5 py-2" onMouseEnter={()=>setHighlighted(index)}
    >
      <div className="flex flex-col gap-1 items-center ">
        <div className={cn("rounded-full p-[2px] border-[2px] border-transparent",highlighted && "border-primary")}>
          <div className=
          {cn(" border-primary border-[2px] p-[3px] rounded-full",highlighted && "bg-primary")}></div>
        </div>
        <div className="w-[2px] h-full bg-primary"/>
      </div>
      <div className={cn("bg-gray-200 rounded-3xl p-4 px-6 col-span-9",highlighted && "text-white bg-primary")}>
        <div className="flex justify-between py-1 gap-4">
          <p className="text-md font-bold truncate select-none">{title}</p>
          <Typography.Info className={cn("text-sm text-gray-600 select-none",highlighted && "text-white")}>{time}</Typography.Info>
        </div>
        <Typography.Info className={cn("text-sm text-gray-600 select-none",highlighted && "text-white")}>{desc}</Typography.Info>
      </div>
    </div>
  )
}
