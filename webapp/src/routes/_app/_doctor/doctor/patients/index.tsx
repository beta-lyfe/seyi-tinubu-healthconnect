  ...
  return (
    <Card>
      ...
      <div className="flex items-center gap-4">
        ...
        <Button className="bg-transparent text-primary hover:text-primary hover:bg-transparent">
          See All
        </Button>
      </div>
    </Card>
  ...
  return (
    <Card>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
        {data.map((item, index) => (
          <OverViewComponent {...item} key={index} />
        ))}
      </div>
    </Card>
  )
...
function Others() {
  ...
  const OthersComponent = ({ Icon, label }: { Icon: any; label: string }) => (
    <div
      className="flex flex-col md:flex-row flex-[0.5]
        hover:bg-primary
        group
        justify-between
    ...
    >
      ...
      <p className="font-semibold text-[13px] text-black group-[]:group-hover:text-white text-nowrap">
        {label}
      </p>
  ...
  return (
    <Card>
      <div className="grid grid-cols-2 gap-6">
        {data.map((index, _) => (
          <OthersComponent Icon={index.Icon} label={index.label} key={_} />
        ))}
      </div>
    </Card>
  )
